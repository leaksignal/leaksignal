...
sidebar_position: 3
---
Ratelimits allow specific endpoints, services, and clusters to ratelimit on individual ips, tokens, or service names.

## Format

Ratelimits are specified in the top-level `ratelimits` field of the policy.
...
* `by`: `ip` (default), `token`, `service` -- the unique ID to be used for ratelimiting. Note that response-derived tokens will still let the initial request through.
