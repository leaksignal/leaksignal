...
### alert
The `alert` field specifies when the values matched by this rule are exceptional and require an alert. Only functional when policy is served through LeakSignal Command.

#### Format
The `alert` field is an object with the following fields:
* `per_request`: Sets the minimum number of unique matches required within a single response to fire an alert.
* `per_5min_by_ip`: Sets the minimum number of unique matches by a unique IP within a 5 minute span to fire an alert. If the `report_style` is `none`, then any matches are considered unique.
* `per_5min_by_token`: Sets the minimum number of unique matches by a unique token within a 5 minute span to fire an alert. If the `report_style` is `none`, then any matches are considered unique.
