...
Where `$LEAKSIGNAL_VERSION` is the version of LeakSignal (i.e. 0.10.0), and `$NGINX_VERSION` is a version between NGINX 1.21.6 and 1.25.2.
...
Tags are of the form: `$INGRESS_VERSION-$LEAKSIGNAL_VERSION`, i.e. `v1.8.1-0.10.0`. Supported versions are 1.6.4 through 1.8.1.
